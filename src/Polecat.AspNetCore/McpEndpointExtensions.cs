...
public static class McpEndpointExtensions
{
    ...
    /// <param name="pattern">The route prefix for MCP endpoints. Defaults to "/polecat/mcp".</param>
    /// <returns>A <see cref="RouteGroupBuilder"/> for further configuration (e.g. RequireAuthorization)</returns>
    public static RouteGroupBuilder MapPolecatMcp(
        this IEndpointRouteBuilder endpoints,
        string pattern = "/polecat/mcp")
    {
    ...
    private static Task HandleToolsList(HttpContext context, JsonRpcRequest request)
    {
        var tools = new[]
        {
            new McpToolDefinition
            {
                ...
                Description =
                    "Returns the current Polecat event store configuration including stream identity, tenancy style, schema name, and metadata settings.",
            ...
            new McpToolDefinition
            {
                ...
                Description =
                    "Lists all event types registered with the Polecat event store, including their type name aliases and .NET type names.",
                InputSchema = new McpToolInputSchema
                {
                    Type = "object", Properties = new Dictionary<string, object>()
                }
            ...
            new McpToolDefinition
            {
                ...
                Description =
                    "Lists all projections and subscriptions configured in the Polecat event store, including their lifecycle and shard information.",
                InputSchema = new McpToolInputSchema
                {
                    Type = "object", Properties = new Dictionary<string, object>()
                }
    ...
    private static Task HandleToolsCall(HttpContext context, JsonRpcRequest request)
    {
        ...
        {
            "get_event_store_configuration" => HandleGetEventStoreConfiguration(context, request, store),
            "list_known_event_types" => HandleListKnownEventTypes(context, request, store),
            "list_projections" => HandleListProjections(context, request, store),
            _ => WriteJsonRpcError(context, request.Id, -32602, $"Unknown tool: {toolName}")
        };
    }

    private static Task HandleGetEventStoreConfiguration(
        HttpContext context, JsonRpcRequest request, IDocumentStore store)
    {
        var events = store.Options.Events;
        var config = new Dictionary<string, object>
        {
            ["streamIdentity"] = events.StreamIdentity.ToString(),
            ["tenancyStyle"] = events.TenancyStyle.ToString(),
            ["databaseSchemaName"] = events.DatabaseSchemaName ?? store.Options.DatabaseSchemaName,
            ["enableCorrelationId"] = events.EnableCorrelationId,
            ["enableCausationId"] = events.EnableCausationId,
            ["enableHeaders"] = events.EnableHeaders
        };
    ...
    private static Task HandleListKnownEventTypes(
        HttpContext context, JsonRpcRequest request, IDocumentStore store)
    {
        var eventGraph = store.Options.EventGraph;
        var eventTypes = eventGraph.AllKnownEventTypes()
            .Select(et => new Dictionary<string, string>
            {
