...
If you prefer to keep your aggregate free of Polecat-specific attributes, you can configure natural keys in a projection class:
...
The `NaturalKey()` method identifies the property, and `SetBy<TEvent>()` tells Polecat which events carry a new value for the key.

## Event-to-Key Mappings

Every event type that sets or changes the natural key must be declared either through the `[NaturalKeySource]` attribute or the fluent `SetBy<TEvent>()` call. When Polecat processes events during an append operation, it extracts the key value from these mapped events and writes it to the lookup table.
...
Natural keys can change over the lifetime of a stream. When an event mapped with `[NaturalKeySource]` or `SetBy<TEvent>()` is appended, Polecat updates the lookup table with the new value using a `MERGE` statement. The old key value is replaced, so lookups using the previous key will no longer resolve to that stream.
